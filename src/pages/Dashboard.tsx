...
export function Dashboard() {
  ...
  const { stats, loading: statsLoading, error: statsError, currentBatch } = useDashboardStats();
  const { batches, error: batchesError } = useImportBatches();
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="card p-8 max-w-md text-center">
          ...
          <p className="text-slate-400 mb-4">{error?.message || 'Failed to load dashboard data'}</p>
          <p className="text-sm text-slate-500 mb-6">This may be a database connection or permission issue. Try refreshing the page or check Supabase configuration.</p>
